...
impl TapeBuilder {
    fn constant(&mut self, v: f64) -> Slot {
        self.insts.push(Instr::Const(v))
    }

    fn unary(&mut self, op: UnaryOp, a: Slot) -> Slot {
    ...
    fn binary(&mut self, op: BinaryOp, a: Slot, b: Slot) -> Slot {
        use Instr::Const;
...
pub fn gradient(tape: &Tape, output_idx: usize, n_vars: u32) -> Tape {
    ...
    let mut tb = TapeBuilder { insts: tape.insts.clone() };

    // adj[s] = slot currently holding d(output)/d(s), or None if zero so far.
    let mut adj: IndexVec<Slot, Option<Slot>> =
        index_vec::index_vec![None; tape.insts.len()];

    let seed = tb.constant(1.0);
    adj[tape.outputs[output_idx]] = Some(seed);

    for (slot, &inst) in tape.insts.iter_enumerated().rev() {
        ...
        match inst {
            ...
            Instr::Unary(op, a) => {
                ...
                accumulate(&mut tb, &mut adj, a, contrib);
            }

            Instr::Binary(op, lhs, rhs) => match op {
                BinaryOp::Add => {
                    accumulate(&mut tb, &mut adj, lhs, d);
                    accumulate(&mut tb, &mut adj, rhs, d);
                }
                BinaryOp::Sub => {
                    accumulate(&mut tb, &mut adj, lhs, d);
                    let nd = tb.unary(UnaryOp::Neg, d);
                    accumulate(&mut tb, &mut adj, rhs, nd);
                }
                BinaryOp::Mul => {
                    let ca = tb.binary(BinaryOp::Mul, d, rhs);
                    accumulate(&mut tb, &mut adj, lhs, ca);
                    let cb = tb.binary(BinaryOp::Mul, d, lhs);
                    accumulate(&mut tb, &mut adj, rhs, cb);
                }
                BinaryOp::Div => {
    ...
    // Collect input adjoints, in VarId order. Inputs the output doesn't
    // depend on get an explicit zero.
